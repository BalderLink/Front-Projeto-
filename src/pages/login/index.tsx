  ...
  return (
    <main className="text-black bg-gray-200 flex">
      ...
      <section className="flex-col justify-center items-center flex p-32 w-1/2">
        ...
        <input onChange={(e) => { setNome(e.target.value) }} value={nome} className="p-2 m-3 rounded-lg w-full" type="text" placeholder="nome" />
        <input onChange={(e) => { setSenha(e.target.value) }} value={senha} className="p-2 m-3 rounded-lg w-full" type="text" placeholder="senha" />
        <div className="flex flex-row">
          <button type="submit" className="transition-transform duration-300 hover:scale-110 p-2 m-3 bg-green-300 rounded-lg">Entrar</button>
          <button className="transition-transform duration-300 hover:scale-110 p-2 m-3 bg-green-300 rounded-lg">Criar conta</button>
        </div>
        </form>
      </section>
